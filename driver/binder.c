			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	//分配两个结构体内存
	// 第 4 步：生成一个 binder_transaction,用于描述本次要进行的 transaction
	...
	//分配两个结构体内存
	// 第 5 步：生成一个 binder_work 变量，用于说明当前调用者线程由一个未完成的 transaction
	...
	//从target_proc分配一块buffer, 这里实际申请目标进程的物理内存，用于准备容纳所有传输的数据
	...
	//第 7 步：申请到 t->buffer 后。从用户空间将数据拷贝进来，这里是真正发生 一次拷贝 传输数据的 地方 ！！！
	...
	list_add_tail(&tcomplete->entry, &thread->todo); //记录当前线程由一个未完成的操作
	if (target_wait)
